...
Brick::update(void) 
{
  ...
  static Vector strain(nstress);  //strain
  static Vector strainRate(nstress);  //strainRate
  ...
  for ( i = 0; i < numberGauss; i++ ) {
    ...
    for ( j = 0; j < numberNodes; j++ )  {
      ...
      ***************************************************/
	  ...
	  strainRate(0) += b00 * vl0;
	  strainRate(1) += b11 * vl1;
	  strainRate(2) += b22 * vl2;
	  strainRate(3) += b30 * vl0 + b31 * vl1;
	  strainRate(4) += b41 * vl1 + b42 * vl2;
	  strainRate(5) += b50 * vl0 + b52 * vl2;

    } // end for j
    
    //send the strain to the material 
    success = materialPointers[i]->setTrialStrain( strain , strainRate ) ;
