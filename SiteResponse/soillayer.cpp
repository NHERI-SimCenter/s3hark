...
SoilLayer::SoilLayer(std::string name, double thickness, double vs, double vp, double rho, double Su, double hG, double m, int numEle, std::string mat, std::string elementType) :
	sl_name(name),
	sl_thickness(thickness),
	sl_vs(vs), sl_vp(vp), sl_su(Su),
	sl_rho(rho), sl_hG(hG), sl_m(m),
	sl_numElement(numEle), sl_Material(mat),sl_Element(elementType)
{
