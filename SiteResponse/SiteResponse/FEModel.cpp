...
SiteResponseModel::SiteResponseModel(SiteLayering layering, std::string modelType, OutcropMotion* motionX, OutcropMotion* motionY) :
...
SiteResponseModel::SiteResponseModel(SiteLayering layering, std::string modelType, OutcropMotion* motionX) :
	SRM_layering(layering),
	theModelType(modelType),
	theMotionX(motionX),
	theOutputDir(".")
...
SiteResponseModel::runTotalStressModel()
{
	...
	for (int layerCount = numLayers - 2; layerCount > -1; --layerCount)
	{
		...
		for (int nodeCount = 0; nodeCount < layerNumNodes[layerCount]; nodeCount += numNodesPerLayer)
		{
			...
			{//2D
				theNode = new Node(nCount + nodeCount + 1, 2, 0.0, yCoord); theDomain->addNode(theNode);
				theNode = new Node(nCount + nodeCount + 2, 2, 1.0, yCoord); theDomain->addNode(theNode);
			} else {//3D
	...
	{
		theNode = new Node(numNodes + 1, 2, 0.0, 0.0); theDomain->addNode(theNode); // TODO ?
		theNode = new Node(numNodes + 2, 2, 0.0, 0.0); theDomain->addNode(theNode); // TODO ?
	...
	for (int i_remove=0; i_remove<sizeTheSPtoRemove;i_remove++)
		{theSP = theDomain->removeSP_Constraint(theSPtoRemove(i_remove)); delete theSP;}
	/*
	...
	*/
	...
	Vector x(numDimDashpot);  Vector y(numDimDashpot); 
	if (!theModelType.compare("2D"))//2D
	{
		x(0) = 1.0; x(1) = 0.0; x(2) = 0.0;
		y(1) = 1.0; y(0) = 0.0; y(2) = 0.0;
