...
SiteLayering::readFromFile(const char* layerFileName)
{
	...
	while (getline(layerFile, line))
	{
		...
		double thick, vs, vp, rho, su, hG, m, numElement;
		std::string Material, Element;
		std::istringstream lines(line);
		lines >> layerName >> thick >> vs >> vp >> rho >> su >> hG >> m >> numElement >> Material >> Element;
		//std::cout << "Layer : " << layerName << ", " << thick << ", " << vs << ", " << vp << ", " << rho << ", " << su << ", " << hG << ", " << m << std::endl;

		SoilLayer newLayer(layerName, thick, vs, vp, rho, su, hG, m, numElement, Material, Element);
